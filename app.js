...
async function fetchWithRetryDownload(url, abortController) {
  ...
  while (attempts < maxAttempts) {
    try {
      const timeout = 25000;
      const response = await fetch(url, { signal: abortController.signal, mode: 'cors' });
      if (!response.ok) throw new Error(`Respuesta de red no OK: ${response.status}`);
      return response;
    } catch (error) {
...
async function downloadAsBlob(url, onProgress, abortController) {
  try {
    const response = await fetchWithRetry(url, { signal: abortController.signal, mode: 'cors' });
    if (!response.body) {
    ...
    throw new Error(`Error descargando el archivo: ${err.message}`);
  }
}
    ...
    downloadBtn.addEventListener("click", async (ev) => {
      ...
      req.onsuccess = async (e) => {
        ...
        try {
          ...
          // *** ZONA EDITADA: Usar la nueva API para descargas offline ***
          const apiUrl = `https://media-hub-player.vercel.app/api/ytmp3?url=https://youtube.com/watch?v=${videoId}`;
          const resApi = await fetchWithRetryDownload(apiUrl, abortController);
          const jsonData = await resApi.json();
          if (jsonData.status !== 200 || !jsonData.url) {
            throw new Error(`Error en la conversión: ${jsonData.message || "Respuesta inválida"}`);
          }
          ...
          saveSongToDB({ videoId, title: jsonData.title || title, thumbnail: thumb, blob, coverBlob, duration, downloadTime: Date.now() });
          ...
          if (err.name === 'AbortError') {
            ...
            showToast(`Error al descargar la canción: ${err.message}`);
            downloadBtn.innerHTML = "Descargar offline";
            downloadBtn.style.display = "inline-block";
            progressContainer.style.display = "none";
          }
...
async function loadAndPlayCurrent() {
  ...
  try {
    // *** ZONA EDITADA: Usar la nueva API para reproducción en línea ***
    const apiUrl = `https://media-hub-player.vercel.app/api/ytmp3?url=https://youtube.com/watch?v=${track.videoId}`;
    const resApi = await fetchWithRetry(apiUrl, {
      headers: { 'accept': 'application/json' }
    });
    const jsonData = await resApi.json();
    // Validar la respuesta de la nueva API
    if (jsonData.status !== 200 || !jsonData.url) {
      throw new Error(`Error obteniendo enlace de streaming: ${jsonData.message || "Respuesta inválida"}`);
    }
    track.audioUrl = jsonData.url;
    ...
    showToast(`Error reproduciendo la canción: ${err.message}`);
    if (currentIndex < playQueue.length - 1) {
      currentIndex++;
      loadAndPlayCurrent();
    } else {
...
function startPlayback() {
  ...
  // Iniciar preloads para pistas cercanas después de un breve retraso
  setTimeout(preloadNearbyTracks, 2000);
}
...
function updateMediaSession(track) {
  if ('mediaSession' in navigator) {
    ...
    // Soporte adicional para seek en MediaSession
...
audio.addEventListener("timeupdate", () => {
  if (audio.duration) {
    ...
    // Actualizar posición en MediaSession para mejor integración en background
    if ('mediaSession' in navigator) {
...
// Función para preloaded de pistas cercanas (estrategia principal para background playback)
async function preloadNearbyTracks() {
  // Preload next 2 tracks and previous 1 if applicable
  const indicesToPreload = [];
  if (currentIndex > 0) indicesToPreload.push(currentIndex - 1);
  if (currentIndex + 1 < playQueue.length) indicesToPreload.push(currentIndex + 1);
  if (currentIndex + 2 < playQueue.length) indicesToPreload.push(currentIndex + 2);

  for (const index of indicesToPreload) {
    await preloadTrack(index);
  }
  // Limpiar blobs no cercanos para optimizar memoria
  cleanUpPreloadedBlobs();
}
...
async function preloadTrack(index) {
  ...
  if (!track.useStreamApi || (track.audioUrl && track.audioUrl.startsWith("blob:"))) return; // Ya local o preloaded

  const controller = new AbortController();
  preloadAbortControllers.set(index, controller);

  try {
    const apiUrl = `https://media-hub-player.vercel.app/api/ytmp3?url=https://youtube.com/watch?v=${track.videoId}`;
    const res = await fetchWithRetry(apiUrl, { signal: controller.signal });
    const json = await res.json();
    if (json.status !== 200 || !json.url) return;

    const blob = await downloadAsBlob(json.url, null, controller);
    const blobUrl = URL.createObjectURL(blob);
    track.audioUrl = blobUrl;
    track.useStreamApi = false;
    preloadedBlobs.add(blobUrl);
  } catch (err) {
    if (err.name !== 'AbortError') {
      console.error(`Preload failed for track ${index}:`, err);
    }
  } finally {
    preloadAbortControllers.delete(index);
  }
}

// Función para limpiar blobs preloaded no necesarios (optimización de memoria)
function cleanUpPreloadedBlobs() {
  const nearbyIndices = new Set([currentIndex - 1, currentIndex, currentIndex + 1, currentIndex + 2].filter(i => i >= 0 && i < playQueue.length));
  preloadedBlobs.forEach(url => {
    const track = playQueue.find(t => t.audioUrl === url);
    if (track) {
      const index = playQueue.indexOf(track);
      if (!nearbyIndices.has(index)) {
        URL.revokeObjectURL(url);
        preloadedBlobs.delete(url);
        track.audioUrl = null; // Reset to force re-fetch if needed later
        track.useStreamApi = true;
      }
    }
  });
}

// Manejo de visibilidad para pausar/resumir preloads si es necesario (estrategia adicional)
document.addEventListener("visibilitychange", () => {
  if (document.visibilityState === "hidden") {
    // App en background: pausar preloads activos para ahorrar batería/data
    preloadAbortControllers.forEach(controller => controller.abort());
    preloadAbortControllers.clear();
  } else {
    // App visible: reiniciar preloads si es necesario
    if (isPlaying) {
      preloadNearbyTracks();
    }
  }
